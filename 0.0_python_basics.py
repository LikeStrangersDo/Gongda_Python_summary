...
print("L" in str) # check if element "B" is in the string  

# 2> replace part of the string
str_1 = "Birmingham Supersite NOx"
str_2 = str_1.replace("NOx","NO2")
print(str_2)

# 3> split any string: string.split(separator, maxsplit) 
...
print("{0}\n{1}".format(data,date))
#####################################################################################################
# list and list compreshension as a shorten format of for loops
...
a[2] = "x"       # assign a[2]
b = a + [5,6,7]
c = [x*2 for x in b] # do the same operation for each element in the list

for x in [a,b,c]:
    print(x)
    
# example 2
a = ['Birmingham','Leicester','London','York','Edinburgh']
print(a[2:4])
print(a[-2:])
...
#####################################################################################################
# dictionary (a conventional way of storing information)
...
A and B # It means "&", but "and" can be faster than "&". For example, if the first condition if not met, it will skip the current iteration.
A or B  # "|"
not A  
...
# "range" in Python is different from that in R, and Python counts from 0 while it does not keep the max value in the range
range(n)      # 0,1,2...n-1
range(1,n)    # 1,2,3...n-1
