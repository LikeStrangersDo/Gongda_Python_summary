...
It is worth focusing on Python. An important reason is that you should be able to resolve all your research computing problems with Python alone. Python in general has a very large user community, so that you can easily seek support. Python is also widely used in our field. Once you have managed Python, you can also develop applications for almost any other tasks outside our field using Python, as it is a general-purpose programming language. 

Python is not perfect, it is faster than R, while still a lot slower than languages like C and Fortran. But languages like C and Fortran are not good for data visualsations and many other tasks in research. Their syntax are also not readable, making it harder to maintain your codes and commmunicate your issues. Julia combines the speed of Fortran and the readable syntax of Python. But currently Julia is not as mature as Python, while a lot of Python packages have been well developed (Python is particularly smart in handling NetCDF files which are freqently used in our research). 
